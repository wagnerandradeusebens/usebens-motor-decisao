// Types mirroring the .NET backend contracts (MotorDecisao.*). Enum string values
// match the C# enum names, which the API serializes/accepts as strings.
...
/** Read-only external source catalog (GET /sources). */
...
/** One node in the deep resolution tree of a formula. */
...
/** Block/category a trace step belongs to, for grouping the log. */
...
/** Node kind config payloads (serialized into GraphNode.config as JSON). */
...
/** A global (cross-policy) reusable variable. */
